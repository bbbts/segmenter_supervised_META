...
def inference(
    ...
    """
    Perform sliding-window inference over a list of images.

    Args:
        model: segmentation model
        ims: torch.Tensor of images, shape [B, C, H, W] or [C, H, W]
        ims_metas: list of metadata dicts for each image
        ori_shape: original image shape (H, W)
        window_size: int, size of sliding window
        window_stride: int, stride of sliding window
        batch_size: int, batch size for forward passes

    Returns:
        seg_map: torch.Tensor, shape [C, ori_H, ori_W]
    """
    C = model.n_cls
    ...
    for im, im_metas in zip(ims, ims_metas):
        ...
        # ---- Ensure 4D tensor ----
        if im.ndim == 3:  # [C, H, W] ? [1, C, H, W]
            im = im.unsqueeze(0)
        elif im.ndim == 2:  # [H, W] ? [1, 1, H, W]
            im = im.unsqueeze(0).unsqueeze(0)

        # ---- Resize image if smaller than window_size ----
        im = resize(im, window_size)

        # ---- Sliding window ----
        flip = im_metas.get("flip", False)
        windows = sliding_window(im, flip, window_size, window_stride)
        crops = torch.stack(windows.pop("crop"))[:, 0]  # [num_windows, C, ws, ws]
    ...
    # ---- Average over ims (if multiple images passed) ----
    seg_map /= len(ims)
    return seg_map
